...
begin
 ...
 RAM(to_integer(unsigned(RAM_ADDR_1))) <= RAM_DATA_IN_1;
 ...
 if(rising_edge(RAM_CLOCK)) then
    ...
    -- write input data into RAM at the provided address
    RAM(to_integer(unsigned(RAM_ADDR_2))) <= RAM_DATA_IN_2;
    -- The index of the RAM array type needs to be integer so
    -- converts RAM_ADDR from std_logic_vector -> Unsigned -> Interger using numeric_std library
    end if;
    end if;
 ...
 -- Data to be read out 
 RAM_DATA_OUT_1 <= RAM(to_integer(unsigned(RAM_ADDR_1)));
 RAM_DATA_OUT_2 <= RAM(to_integer(unsigned(RAM_ADDR_2)));
end Behavioral;